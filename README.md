...
## To build:
`podman build --tag llamaswap -f Containerfile --device nvidia.com/gpu=all --security-opt=label=disable .`
...
### Example execution:
`podman run -d --device nvidia.com/gpu=all --security-opt=label=disable -v ~/LLM/models:/var/lib/models -v ~/LLM/config/llama-swap.config.yaml:/etc/llama-swap/config.yaml -p 9000:9000 llamaswap`
